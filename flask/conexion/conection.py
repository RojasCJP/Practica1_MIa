...
DB_USER = 'postgres'
DB_PASS = 'password'
CONSULTA1 = """select count(*) from Inventario 
...
order by nombre;"""
CONSULTA4 = """select distinct entrega.lanzamiento, actor.nombre, actor.apellido from actorentrega 
...
CONSULTA6 = """with CLIENTESCIUDAD as(
...
)select RENTAPAIS.nombre as nombre_pais, RENTACIUDAD.nombre as nombre_ciudad, cast(RENTACIUDAD.rentas as float)/CIUDADESPAIS.cantidad as promedio
from RENTAPAIS
...
inner join RENTACIUDAD on RENTACIUDAD.id_pais = RENTAPAIS.id_pais
inner join CIUDADESPAIS on CIUDADESPAIS.id_pais = RENTAPAIS.id_pais;"""
CONSULTA8 = """with RENTASTOTALES as(
    select count(renta.monto_pagar) as rentas, pais.nombre, pais.id_pais from renta
    inner join cliente on cliente.id_cliente = renta.id_cliente
    inner join direccion on cliente.id_direccion = direccion.id_direccion
    inner join ciudad on direccion.id_ciudad = ciudad.id_ciudad
...
CONSULTA9 = """with CIUDADESLISTADO as(
...
ELIMINARTEMPORAL = """delete from temporal;"""
ELIMINARMODELO = """drop table ActorEntrega;
drop table Actor;
...
async def conection_db(consulta):
    ...
    select = 0
    select = selects(consulta, conection)
    conection.commit()
    conection.close()
    return select
